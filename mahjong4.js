...
// game.js — 四人麻將主遊戲邏輯
//
// 負責：遊戲狀態管理、牌局流程、玩家/AI 動作處理、
//       UI 渲染（手牌/棄牌/副露/花牌）、胡牌役種判斷、
//       骰子決莊家、分數計算、BGM/音效控制
//
// 依賴：mahjong-core.js（window.MJ）
// 對應 HTML：index.html（桌面版）、mobile.html（手機版）
// ══════════════════════════════════════════════════════════════

const MJ = window.MJ;

/* ── 玩家/AI 座位 ID 常量 ── */
const PLAYER_SOUTH = "S"; // 南家（玩家）
const AI_EAST  = "E";     // 東家 AI
const AI_SOUTH = "S_AI";  // 未使用，保留
const AI_WEST  = "W";     // 西家 AI
const AI_NORTH = "N";     // 北家 AI

// ── 座位全名（骰子/日誌顯示用）── 須在 PLAYER_SOUTH/AI_EAST/AI_WEST/AI_NORTH 之後定義
...
const DIE_CHARS = ["⚀","⚁","⚂","⚃","⚄","⚅"]; // Unicode 骰子字元
// 骰子順序：索引 0=南, 1=東, 2=北, 3=西（依骰子點數 mod 4 對應座位）
const DICE_SEAT_ORDER = [PLAYER_SOUTH, AI_EAST, AI_NORTH, AI_WEST];

/* ══════════════════════════════════════════════════════════════
   全局遊戲狀態
   ══════════════════════════════════════════════════════════════ */
let wall = [];          // 剩餘牌山
let hands = {           // 四家手牌（含摸到的牌）
  [PLAYER_SOUTH]: [],
  [AI_EAST]: [],
  [AI_WEST]: [],
  [AI_NORTH]: [],
...
/* ══════════════════════════════════════════════════════════════
   DOM 元素快取（啟動時一次性取得，避免重複 querySelector）
   ══════════════════════════════════════════════════════════════ */
...
const elAiNorthHand = document.getElementById("ai-north-hand");
...
const elDiscardNorth = document.getElementById("discard-north");
...
const elFlowersNorth = document.getElementById("flowers-north");
...
const elFeisNorth = document.getElementById("feis-north");
...
const elMeldsNorth = document.getElementById("melds-north");
...
btnChi.addEventListener("click", onPlayerChi);
...
// 主頁右側菜單：遊戲介面預覽
document.getElementById("btn-home-preview").addEventListener("click", () => {
  ...
  const _savedState = {
    wall:       [...wall],
    hands:      { S: [...hands[PLAYER_SOUTH]], E: [...hands[AI_EAST]], W: [...hands[AI_WEST]], N: [...hands[AI_NORTH]] },
    discards:   { S: [...discards[PLAYER_SOUTH]], E: [...discards[AI_EAST]], W: [...discards[AI_WEST]], N: [...discards[AI_NORTH]] },
    flowers:    { S: [...flowers[PLAYER_SOUTH]], E: [...flowers[AI_EAST]], W: [...flowers[AI_WEST]], N: [...flowers[AI_NORTH]] },
    melds:      { S: [...melds[PLAYER_SOUTH]], E: [...melds[AI_EAST]], W: [...melds[AI_WEST]], N: [...melds[AI_NORTH]] },
    currentTurn, dealerPlayer, drawnThisTurn, gameOver, awaitingPlayerClaim, lastDiscard,
  };

  // ── 2. 注入示例牌面 ──
  const DEMO_SOUTH = ["M1","M2","M3","P4","P5","P6","S7","S8","S9","Z1","Z1","Z2","Z2","M9"];
  const DEMO_DISC_S = ["M4","P1","S3","Z3","M7"];
  const DEMO_DISC_E = ["P2","S5","M6","Z4","P9"];
  const DEMO_DISC_W = ["S1","M3","P7","Z1","S6"];
  const DEMO_DISC_N = ["M5","P3","S2","Z2","M8"];
  hands[PLAYER_SOUTH] = [...DEMO_SOUTH];
  hands[AI_EAST]  = Array(13).fill("?");
  hands[AI_WEST]  = Array(13).fill("?");
  hands[AI_NORTH] = Array(13).fill("?");
  discards[PLAYER_SOUTH] = DEMO_DISC_S;
  discards[AI_EAST]      = DEMO_DISC_E;
  discards[AI_WEST]      = DEMO_DISC_W;
  discards[AI_NORTH]     = DEMO_DISC_N;
  flowers[PLAYER_SOUTH] = ["F1"];
  flowers[AI_EAST] = flowers[AI_WEST] = flowers[AI_NORTH] = [];
  melds[PLAYER_SOUTH] = melds[AI_EAST] = melds[AI_WEST] = melds[AI_NORTH] = [];
  wall = Array(60).fill("?");
  ...
  hands[AI_NORTH] = _savedState.hands.N;
...
document.getElementById("game-preview-modal").addEventListener("click", (e) => {
...
(function() {
  ...
  // 用真實 renderTile 填充所有 [data-tiles] 牌行（只填一次）
  ...
  },
  {
    ...
    items: [
      { name:"三人對局",    cls:"yr-special", desc:"東・南・西 三家對局，無北家位置",
        tiles:[["Z1","Z2","Z3"]] },
      { name:"無北風牌",    cls:"",           desc:"牌局中移除所有北風牌（Z4），共少 4 張，牌山較薄",
        tiles:[["Z1","Z2","Z3"]] },
      { name:"無吃牌",      cls:"",           desc:"三人麻將通常禁止吃牌（吃），僅可碰、槓、胡",
        tiles:[] },
      { name:"自摸加倍",    cls:"yr-special", desc:"自摸時，兩家各賠全額（非平分）",
        tiles:[] },
      { name:"榮胡只賠一家",cls:"",           desc:"榮胡時僅打牌者賠付，非打牌者不賠",
        tiles:[] },
...
function showTileRefOverlay() {
  ...
  const GROUPS = [
    {
      label: "萬子 M1–M9",
      tiles: Array.from({ length: 9 }, (_, i) => `M${i + 1}`),
    },
    {
      label: "筒子 P1–P9",
    ...
    {
      label: "索子 S1–S9",
      tiles: Array.from({ length: 9 }, (_, i) => `S${i + 1}`),
    },
    {
      label: "字牌　東 南 西 北 中 發 白",
      tiles: Array.from({ length: 7 }, (_, i) => `Z${i + 1}`),
    },
    {
      label: "花牌 F1–F16　春夏秋冬 梅蘭竹菊 琴棋書畫 貓雞蟲鼠",
      tiles: Array.from({ length: 16 }, (_, i) => `F${i + 1}`),
    },
    {
      label: "飛牌（萬用牌）H1–H4",
      tiles: ["H1", "H2", "H3", "H4"],
...
document.getElementById("btn-exit").addEventListener("click", () => {
  ...
  document.getElementById("win-overlay").classList.add("hidden");
  document.getElementById("dice-overlay").classList.add("hidden");
  document.getElementById("chi-select-overlay").classList.add("hidden");
  document.getElementById("action-bar").style.display = "none";
  document.getElementById("home-screen").classList.remove("hidden");
  hideTopBar();
  refreshProfileCard();
...
function classifyWinningHand(handTiles, openMelds, decomposedGroups, ctx = {}) {
  ...
  // ── 十三幺（無副露）─────────────────────────────────────────────
  if (openMelds.length === 0 && handTiles) {
    const orphans = ["M1","M9","P1","P9","S1","S9","Z1","Z2","Z3","Z4","Z5","Z6","Z7"];
    const raw13 = handTiles.filter(t => t[0] !== "H" && t[0] !== "F");
    if (orphans.every(o => raw13.includes(o)))
      yaku.push({ name: "十三幺", cls: "yaku-special" });
  }
...
function findTenpaiOptions(hand) {
  ...
  const visibleCount = {};
  const allPlayers = [PLAYER_SOUTH, AI_EAST, AI_WEST, AI_NORTH];
...
function calculateAndApplyPayout(winner, totalFan, isRonghu, discardFrom, fanReductions = {}, duifeiBonus = 0) {
  const all = [PLAYER_SOUTH, AI_EAST, AI_WEST, AI_NORTH];
  ...
  let winnerGain = 0;
  if (isRonghu) {
    // 榮胡：打牌者賠主番 × 3；三家各額外賠對飛番
    const from = discardFrom ?? null;
    all.forEach(pid => {
      if (pid === winner) return;
      let pay = duifeiPay(pid);                         // 對飛：三家各賠
      if (pid === from) pay += effectiveFan(from) * BASE_UNIT * 3 * mult; // 打牌者再賠主番×3
      if (pay > 0) {
        deltas[pid]    -= pay;
        deltas[winner] += pay;
        winnerGain += pay;
...
function showWinOverlay(winner, { winTile = null, isRonghu = false, afterKong = false } = {}) {
  ...
  // 各座位頭像對照
  const SEAT_AVATAR = { S: playerAvatar, E: "🐯", W: "🦁", N: "🤖" };
  ...
  // ── 風位動態計算（庄家 = 東風，按出牌順序遞增）─────
  // 出牌順序：南 → 西 → 北 → 東
  const TURN_ORDER = [PLAYER_SOUTH, AI_WEST, AI_NORTH, AI_EAST];
  const dealerIdx  = TURN_ORDER.indexOf(dealerPlayer);
  const seatPos    = (p) => ((TURN_ORDER.indexOf(p) - dealerIdx + 4) % 4) + 1;
  // 位置 1=東風(Z1) 2=南風(Z2) 3=西風(Z3) 4=北風(Z4)
  const WIND_TILES  = ["Z1", "Z2", "Z3", "Z4"];
  ...
  // 湊齊一組全（F1-F4 / F5-F8 / F9-F12）= 各額外 1 番
  const set1Tiles = [1,2,3,4].map(n => `F${n}`);
  const set2Tiles = [5,6,7,8].map(n => `F${n}`);
  const set3Tiles = [9,10,11,12].map(n => `F${n}`);
  ...
  if ([9,10,11,12].every(n => winnerFlowerNums.includes(n))) addFanTag("琴棋書畫全", 1, "", set3Tiles);
  ...
  if (scoreEl) {
    ...
    if (totalFan > 0) {
      ...
      if (isRonghu) {
        payEl.innerHTML = `榮胡：打牌者賠 <strong>+${(singlePay * 3).toLocaleString()} 分</strong>${duifeiBonus > 0 ? `（另：三家各賠對飛 ${(duifeiBonus * BASE_UNIT * mult).toLocaleString()} 分）` : ""}`;
      } else {
        payEl.innerHTML = `自摸：三家各賠 <strong>${singlePay.toLocaleString()} 分</strong>（共 +${(singlePay * 3).toLocaleString()} 分）`;
      }
  ...
  // ── 全場牌型展示（所有玩家依座位順序） ─────────────────
  allHands.innerHTML = "";
  const DIR_LABEL = { S: "南", E: "東", W: "西", N: "北" };
  const SEAT_ORDER = [PLAYER_SOUTH, AI_EAST, AI_WEST, AI_NORTH];
...
function dealInitialHands() {
  // 標準規則：閒家 13 張，庄家 14 張（庄家起手已算摸了一張）
  const players = [PLAYER_SOUTH, AI_EAST, AI_WEST, AI_NORTH];
  // 輪流發牌，每家先拿 13 張
  for (let i = 0; i < 13; i++) {
  ...
  // 庄家再多發 1 張（14 張起手，可直接出牌）
  if (wall.length > 0) drawOneTileToHandOrFlower(dealerPlayer);
  // 起手排序
  for (let p of players) {
...
function render() {
  ...
  renderAiHand(elAiNorthHand, hands[AI_NORTH].length);
  ...
  renderDiscardRow(elDiscardNorth, discards[AI_NORTH],    AI_NORTH);
  ...
  renderFlowersRow(elFlowersNorth, flowers[AI_NORTH]);
  ...
  renderFlowersRow(elFeisNorth, feis[AI_NORTH]);
  ...
  renderMeldsRow(elMeldsNorth, melds[AI_NORTH]);
  ...
  setBtn(btnChi,     awaitingPlayerClaim && claimOptions.chi);
  ...
  const anyVisible = [btnDraw, btnWin, btnChi, btnPeng, btnGang, btnSwapFei, btnTenpai, btnPass, btnRecommend]
...
// ── 推薦打法 ──────────────────────────────────────────────────────
let _recTile = null; // 目前推薦的牌

/** 清除推薦高亮，並關閉推薦 overlay */
...
// ── 渲染單張牌 DOM 元素 ─────────────────────────────────────────
// tileCode: 牌碼字串，如 "M3"（三萬）/ "P7"（七筒）/ "Z1"（東）/ "F1"（春）
// isPlayer: true = 玩家手牌（較大，可互動）
...
function startRolling() {
  ...
  const iv = setInterval(() => {
    ...
    if (ticks >= totalTicks) {
      ...
      const sum = v1 + v2;
      // 以點數和 mod 4 對應座位（從南家開始順時針）
      const east = DICE_SEAT_ORDER[(sum - 2) % 4];
...
function actuallyStartGame(east) {
  elLog.innerHTML = "";
  log("開始新的一局（庄家 14 張，閒家 13 張）", "system");
  resetState();
  dealerPlayer = east; // resetState 後再設定，避免被覆蓋
  currentTurn  = east; // 庄家先手
  dealInitialHands();
  // 庄家起手已有 14 張（視同已摸牌），直接可以打牌
  drawnThisTurn = true;
  turnCount = 1;
  render();
  log(`${SEAT_NAMES_FULL[east]} 成為本局庄家，手持 14 張，先手出牌。`, "system");

  if (east === PLAYER_SOUTH) {
    // 玩家已有 14 張，直接等待操作（可宣胡或打牌）
  } else {
...
/* ══════════════════════════════════════════════════════════════
   ...
   onPlayerDraw()      — 摸牌
   onPlayerDiscard()   — 棄牌
   onPlayerWinAttempt()— 宣告胡牌
   onPlayerPeng()      — 碰牌
   onPlayerGang()      — 槓牌
   onPlayerChi()       — 吃牌
   onPlayerPass()      — 過牌
   onSwapFei()         — 換飛（用飛替換手牌中一張）
   ══════════════════════════════════════════════════════════════ */
...
function getNextPlayer(player) {
  ...
  if (player === AI_WEST)      return AI_NORTH;
  if (player === AI_NORTH)     return AI_EAST;
  return PLAYER_SOUTH;
}
...
// 搶牌：胡 > 槓 > 碰 > 吃（胡由 endTurnAndNext Phase1 統一處理）
...
// 只處理 AI 槓/碰（不含胡，胡已在 endTurnAndNext Phase1 優先處理）
// 嚴格兩輪掃描：先全場找槓（槓 > 碰），再全場找碰
...
function endTurnAndNext() {
  ...
  if (lastDiscard) {
    ...
    if (from !== PLAYER_SOUTH) {
      ...
      // 槓 優先：AI 可槓 → AI 槓，玩家不能碰/吃
      if (aiGang) {
        awaitingPlayerClaim = false;
        claimOptions = { chi: false, peng: false, gang: false, chiOptions: [] };
        // Fall through to Phase 3 (tryAiClaimsNonRong 會找到槓的 AI)
      }
      // 碰 優先（無人可槓）：玩家可碰 → 只顯示碰按鈕（無吃）
      ...
      // 碰 優先：AI 可碰 → AI 碰，玩家不能吃
      else if (aiPeng) {
        awaitingPlayerClaim = false;
        claimOptions = { chi: false, peng: false, gang: false, chiOptions: [] };
        // Fall through to Phase 3 (tryAiClaimsNonRong 會找到碰的 AI)
      }
      // 吃（最低優先）：無人可槓/碰，玩家可吃
      else if (playerCanChi && awaitingPlayerClaim) {
        // claimOptions already has chi set; keep it
        currentTurn = PLAYER_SOUTH;
        pendingTurnAfterDiscard = naturalNext;
        log(`你可以對 ${getSeatName(from)} 的牌（${formatTileLabel(tile)}）做動作。`, "system");
        render();
    ...
    // ── Phase 4：下家 AI 吃 ──────────────────────────────────────
    if (naturalNext !== PLAYER_SOUTH && canAiChi(naturalNext, tile) && aiShouldChi(naturalNext, tile, from)) {
      setTimeout(() => aiClaimChi(naturalNext, tile, from), 900);
      return;
    }
...
/* ── AI 回合主流程 ───────────────────────────────────────────────
   aiTakeTurn() 由 setTimeout 呼叫（延遲模擬思考時間）
   流程：摸牌 → 檢查自摸胡 → 檢查暗槓 → 選擇棄牌 → 觸發搶牌窗口
   ────────────────────────────────────────────────────────────── */
function aiTakeTurn() {
  if (gameOver) return;
  ...
  if (!drawnThisTurn) {
    ...
    if (!tile) { endGameDraw(); return; }
    log(`${getSeatName(currentTurn)}摸了一張牌。`, "ai");
    afterKongDraw = false;
    turnCount++;
  }
  // 若 drawnThisTurn 已為 true（庄家起手 14 張），直接跳至判斷邏輯
...
function computePlayerClaimOptions() {
  ...
  // 槓：任何人打出的牌都可槓（飛牌不能用於槓）
  if (sameCount >= 3) claimOptions.gang = true;

  // 吃：只限上家（AI_EAST）打出的牌才可吃
  const isFromUpstream = from === AI_EAST; // 上家 = East，因轉序 East→South
  if (isFromUpstream) {
    const suit = tile[0];
    const v = parseInt(tile.slice(1), 10);
    if (suit !== "Z" && suit !== "F" && suit !== "H") {
      const has = (val) => hands[PLAYER_SOUTH].includes(`${suit}${val}`);
      const needFor = (a, b) => {
        let need = 0;
        if (!has(a)) need++;
        if (!has(b)) need++;
        return need <= feiCount;
      };
      const opts = [];
      if (v - 2 >= 1 && needFor(v - 2, v - 1))
        opts.push([`${suit}${v - 2}`, `${suit}${v - 1}`, tile]);
      if (v - 1 >= 1 && v + 1 <= 9 && needFor(v - 1, v + 1))
        opts.push([`${suit}${v - 1}`, tile, `${suit}${v + 1}`]);
      if (v + 2 <= 9 && needFor(v + 1, v + 2))
        opts.push([tile, `${suit}${v + 1}`, `${suit}${v + 2}`]);
      if (opts.length > 0) {
        claimOptions.chi = true;
        claimOptions.chiOptions = opts;
      }
    } else if (suit === "H") {
      // 飛牌作為棄牌被吃：手牌有兩張搭子即可
      for (const s of ["M", "P", "S"]) {
        for (let vv = 1; vv <= 7; vv++) {
          const has = (val) => hands[PLAYER_SOUTH].includes(`${s}${val}`);
          if (has(vv) && has(vv + 1)) { claimOptions.chi = true; break; }
          if (has(vv) && has(vv + 2)) { claimOptions.chi = true; break; }
          if (has(vv + 1) && has(vv + 2)) { claimOptions.chi = true; break; }
        }
        if (claimOptions.chi) break;
      }
    }
  }

  awaitingPlayerClaim = claimOptions.chi || claimOptions.peng || claimOptions.gang;

  // 能胡時，清空吃/碰/槓選項，但保持 awaitingPlayerClaim = true 讓胡按鈕顯示
  if (lastDiscard) {
    const tile = lastDiscard.tile;
    const canRonghu =
      (hands[PLAYER_SOUTH].length + 1) % 3 === 2 &&
    ...
    if (canRonghu) {
      claimOptions = { chi: false, peng: false, gang: false, chiOptions: [] };
      awaitingPlayerClaim = true; // 保持等待狀態，render() 才能顯示胡按鈕
    }
  }
}
...
function onPlayerPass() {
  ...
  if (tile && from) {
    ...
    // 下家 AI 吃
    if (nextPlayer !== PLAYER_SOUTH && canAiChi(nextPlayer, tile)) {
      setTimeout(() => aiClaimChi(nextPlayer, tile, from), 800);
      return;
    }
  }
...
function showDrawOverlay() {
  ...
  if (allHands) {
    ...
    const DIR_LABEL   = { S: "南", E: "東", W: "西", N: "北" };
    const SEAT_AVATAR = { S: playerAvatar, E: "🐯", W: "🦁", N: "🤖" };
    [PLAYER_SOUTH, AI_EAST, AI_WEST, AI_NORTH].forEach(pid => {
      const row = document.createElement("div");
...
/* ══════════════════════════════════════════════════════════════
   工具函式
   getSeatName()       — 座位 ID → 顯示名稱
   speakTile()         — 語音報牌
   formatTileLabel()   — 牌碼 → 人類可讀字串
   log()               — 新增訊息面板記錄
   ══════════════════════════════════════════════════════════════ */
...
document.addEventListener("keydown", (e) => {
  ...
  switch (e.key) {
    ...
    case "c":
    case "C":
      if (isBtnActive(btnChi)) { e.preventDefault(); onPlayerChi(); }
      break;
    case "h":
    case "H":
